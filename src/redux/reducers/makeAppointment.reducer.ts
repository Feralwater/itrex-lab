...
import { AppointmentPending } from '../actions/actions.types';

export const makeAppointmentSlice = createSlice({
  name: 'makeAppointment',
  initialState: { responseStatus: FETCH_STATUS.IDLE } as { responseStatus: Status },
  reducers: {
    fulfilled: (state, action: PayloadAction<AppointmentPending>) => ({ ...state, ...action.payload, responseStatus: FETCH_STATUS.FULFILLED }),
    pending: (state, action: PayloadAction<AppointmentPending>) => ({ ...state, ...action.payload, responseStatus: FETCH_STATUS.LOADING }),
    failed: (state, action: PayloadAction<AppointmentPending>) => ({ ...state, ...action.payload, responseStatus: FETCH_STATUS.FAILED }),
  },
});
